...
<br>This application configures TCE to run in Single-Ramp mode at 20 kHz. TCE generates four Pulse-Width Modulation (PWM) signals with: 15%, 25%, 60% and 75% values for duty cycles.
...
<br>At runtime WEX takes control of the output pins and generates a complementary pattern for 40 ms. Then, WEX gives back control of output pins to TCE who is running and generates four PWM signals with 15%, 25%, 60% and 75% duty cycles. After another 100 ms WEX takes back control of the output pins and activates the Swap feature. For another 40 ms a complementary pattern is set, then WEX gives back control of the output pins to TCE. This time, TCE is running and generates 4 PWM signals with 85%, 75%, 40% and 25% duty cycles. This happens because the Swap feature is active. After another 100 ms the process repeats itself over and over.
...
<br><img src="../images/Add_WEX.png">
    ...
    <br> - Dead time Insertion Channel Enable for all four channels
...
<br>10. Now the project can be built and run from MPLAB X IDE. At run time TCE generates four PWM signals and WEX generates complementary signals, activates the swap feature and can override the port output pins and change the state of each pin from `0` logic to `1` logic or vice-versa.
...
Below are illustrated logic analyzer captures, to help understand better how to override the port pins and take the pins control from TCE using the WEX module:
...
This project shows how to use the WEX to generate different patterns, and take control of the TCE pins during runtime, without stopping the timer. This is extremely useful for applications like Trapezoidal Motor Control, where some PWM signals must be forced to low '0' logic depending on the commutation sector.