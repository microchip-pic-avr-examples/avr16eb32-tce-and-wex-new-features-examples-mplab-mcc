...
<br>After the peripheral clock, the output port pins, TCE and WEX are initialized, the  ``Sinewave_Modulation``` function is called in an infinite loop. This function is registered as a callback and is called every 50us on TCE's Overflow Interrupt Service Routine (ISR).

<br>This application configures TCE to run in single ramp mode at 20KHz. TCE generates one Pulse-Width Modulation (PWM) signal. WEX is configured in Waveform Extension Mode and is able to generate a complementary PWM signal for the one generated by TCE and add dead time in hardware, so the two signals are not overlapping. Dead time values are set to 500 ns both on the low side and high side of the two PWM signals.

<br>The sinusoidal signal is generated using a The Lookup Table (LUT). The values from the LUT are automatically scaled using TCE's scaling hardware accelerator. At runtime the amplitude is initially set to 100% and the frequency is set to 500 Hz. Then after a 1 second delay, the amplitude is set to 50% and the frequency is doubled to 1000 Hz. After another delay of 1 second the process repeats itself over and over again.
...
<br>5. Click on _Project Resources>System>Interrupt Manager_, then do the following configuration:
...
<br>8. In the **Pin Grid View** tab check if the WO0 and WO1 output pins are locked as outputs on PORTA. When the boxes from Enable column from Output Settings of WEX are checked, the pins are also locked. To change the PORT simply click a pin from another PORT in **Pin Grid View**.
...
<br>10. After the MCC Melody generates the project files with the configuration explained above, overwrite the content from the ```main.c``` file with this:
...
<br>11. Now the project can be built and run from MPLAB X IDE. At run time, the generated sinusoidal signal can be modulated in frequency by increasing or decreasing the LUT scrolling speed. Also the sinusoidal signal can be modulated in amplitude by changing the amplitude value in the AMP register of TCE, and the rest is done by the scaling hardware accelerator of TCE.
...
<br><img src="../images/Set_Main_Project.png">
 ...
 4. Build the ```PWM_Modulation.X``` project by clicking on **Clean and Build Project**.
...
Below are illustrations of logic analyzer captures to help understand how the PWM signals look, and how the generated sinewave looks. For visualizing the sinewave, a hardware analog filter was used:
